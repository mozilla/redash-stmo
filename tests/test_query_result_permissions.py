...
from redash_stmo.handlers.query_results import handler_queryresults
...
class TestQueryResultAPI(BaseTestCase):
    ...
    def test_query_result_query(self):
        ...
        restricted_ds = self.factory.create_data_source(group=restricted_group, view_only=False, type='pg')
        upstream_query = self.factory.create_query(user=restricted_user, data_source=restricted_ds, query_text="select * from secret")
        result_ds = self.factory.create_data_source(group=self.factory.org.default_group, view_only=False, type='results')
