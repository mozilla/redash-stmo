...
class STMOBigQuery(BigQuery):
    ...
    def __init__(self, *args, **kwargs):
        super(STMOBigQuery, self).__init__(*args, **kwargs)
        self._query_user = None

    def _get_job_data(self, query):
        job_data = super(STMOBigQuery, self)._get_job_data(query)
        labels = {"App": "redash"}
        ...
        if "Query ID" in parsed_annotation:
            host = base_url(current_org) or "https://sql.telemetry.mozilla.org"
    ...
    def run_query(self, query, user):
        ...
        return super(STMOBigQuery, self).run_query(query, user)
